// static/manager/js/mockup_templates.js
...
document.getElementById('mockup-upload-input')?.addEventListener('change', function(e) {
    ...
    Array.from(files).forEach((file, index) => {
        const id = 'mockup_' + Date.now() + '_' + index; // Yeni olduğu 'mockup_' ile belli
...
async function saveAllToDatabase() {
    ...
    try {
        const response = await fetch('/save-mockups/', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                group_id: currentGroupId, // Yeni grupsa null gider
                group_name: groupName,
                mockups: uploadedMockups // Hem "db_" hem "mockup_" id'li veriler içerir
            })
        });
